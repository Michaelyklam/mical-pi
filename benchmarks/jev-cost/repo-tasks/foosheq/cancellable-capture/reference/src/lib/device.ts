...
function withCancellation<T>(operation: Promise<T>, signal: AbortSignal): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    // Always observe late chooser rejections, including when already aborted.
...
export async function readDeviceProfile(
  ...
  const onAbort = () => controller.abort(abortError(external!));
  external?.addEventListener("abort", onAbort, { once: true });
  try {
    let devices: HIDDevice[];
    try {
      // Keep this call synchronous under the caller's transient user activation.
      devices = await withCancellation(Promise.resolve(hid.requestDevice({
    ...
    try {
      ...
      try {
        ...
        if (captureVerified && controller.signal.aborted) throw abortError(controller.signal);
      }
    }
  } finally {
