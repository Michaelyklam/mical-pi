...
/** Strict editor metadata for durable sessions; legacy imports remain tolerant. */
export function parseEditorProfileStrict(text: string): EditorProfile {
  const profile = parseProfile(text);
  const envelope = JSON.parse(text);
  if (!Object.hasOwn(envelope, "editor")) return profile;
  const editor = envelope.editor;
  if (
    typeof editor !== "object" || editor === null || Array.isArray(editor) ||
    editor.version !== 1 || !Array.isArray(editor.visibleBandIds) ||
    !editor.visibleBandIds.every((id: unknown) => Number.isInteger(id) && Number(id) >= 1 && Number(id) <= 8) ||
    new Set(editor.visibleBandIds).size !== editor.visibleBandIds.length
  ) throw new Error("Invalid version 1 editor membership.");
  return { ...profile, visibleBandIds: visibleBandIds({ ...profile, visibleBandIds: editor.visibleBandIds }) };
}
