...
(function bootstrapWorkflowApi() {
  ...
  function requestAgent(promptValue, optionsValue = {}) {
    ...
    let settled = false;
    let cancelled;
    const cancellationResult = (reason) => ({ ok: false, output: "", error: reason });
    const begin = () => {
      unconsumed.delete(id);
      if (cancelled !== undefined) return Promise.resolve(cancellationResult(cancelled));
      if (!started) {
        let payload;
        try {
        ...
        started = callHost("agent", payload)
          .then((json) => {
            settled = true;
            return cancelled !== undefined ? cancellationResult(cancelled) : JSON.parse(json);
          })
    ...
    return Object.freeze({
      cancel(reason) {
        if (settled || cancelled !== undefined) return false;
        cancelled = typeof reason === "string" && reason.length > 0
          ? reason.slice(0, 1024) : "Agent was cancelled";
        unconsumed.delete(id);
        inFlight.delete(id);
        if (started) callHost("cancelAgent", JSON.stringify({ id, reason: cancelled }));
        return true;
      },
      then(resolve, reject) {
  ...
  try {
    ...
    sandbox.__hostBridge = (kind, payloadJson) => {
      ...
      if (kind === "cancelAgent") {
        ...
        const pending = pendingAgents.get(payload.id);
        pendingAgents.delete(payload.id);
        send({ kind: "cancelAgent", id: payload.id, reason: payload.reason });
        pending?.resolve(JSON.stringify({ ok: false, output: "", error: payload.reason }));
        return undefined;
      }
      if (kind !== "agent")
