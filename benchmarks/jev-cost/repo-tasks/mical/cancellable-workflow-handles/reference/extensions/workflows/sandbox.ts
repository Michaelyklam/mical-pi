...
export function runWorkflowSandbox(options: RunWorkflowSandboxOptions) {
  ...
  return new Promise<unknown>((resolve, reject) => {
    ...
    child.on("message", (raw: unknown) => {
      ...
      if (raw.kind === "cancelAgent") {
        if (typeof raw.id !== "number" || !Number.isSafeInteger(raw.id) || raw.id < 1 ||
            !requestIds.has(raw.id) || typeof raw.reason !== "string" ||
            raw.reason.length === 0 || raw.reason.length > 1024) {
          finish(new Error("Workflow sandbox sent an invalid agent cancellation"));
          return;
        }
        const request = activeAgentRequests.get(raw.id);
        activeAgentRequests.delete(raw.id);
        request?.abort(new Error(raw.reason));
        return;
      }
      if (raw.kind === "phase") {
        if (
      ...
      if (raw.kind === "agent") {
        ...
        void Promise.resolve().then(() => options.onAgent(
            payload.prompt,
