...
test("Pricing uses live direct rates and exact canonical router inheritance", () => {
	const resolver = new PricingResolver([
		...
		model("router-fixture", "bedrock_mantle/gpt-5.6-terra", { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }),
	]);
	const estimate = resolver.estimate("router-fixture", "bedrock_mantle/gpt-5.6-terra", usage(1_000_000, 100_000));
