...
import type { AccountKey, AttributionRecord, ReportedCostRecord, SessionCostSummary } from "./domain.ts";
import { PricingResolver } from "./pricing.ts";

export const ATTRIBUTION_ENTRY = "usage-footer-attribution";

export const REPORTED_COST_ENTRY = "usage-footer-reported-cost";

function isReportedCost(value: unknown): value is ReportedCostRecord {
 if (!value || typeof value !== "object" || Array.isArray(value)) return false;
 const record = value as Record<string, unknown>;
 return ["targetEntryId", "accountKey", "providerId", "source"].every(
  ...
  && typeof record.recordedAt === "number" && Number.isFinite(record.recordedAt) && record.recordedAt >= 0;
}
...
export class SessionLedger {
	...
	summarize(
		...
		const charges = new Map<string, ReportedCostRecord>();
  for (const entry of entries) {
   if (entry.type !== "custom" || entry.customType !== REPORTED_COST_ENTRY || !isReportedCost(entry.data)) continue;
   const record = entry.data;
   if (record.accountKey !== account.accountKey || record.providerId !== account.providerId) continue;
   const previous = charges.get(record.targetEntryId);
   if (!previous || record.recordedAt >= previous.recordedAt) charges.set(record.targetEntryId, record);
  }
  let reported = 0;
  let estimated = 0;
		...
		for (const entry of entries) {
			...
			attributedEntries++;
   const charge = charges.get(entry.id);
   if (charge && charge.providerId === providerId) {
    reported += charge.amount;
    continue;
   }
