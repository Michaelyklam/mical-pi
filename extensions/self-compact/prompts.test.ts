...
test("promptSearchDirs prefers cwd then the extension's vendored prompts dir", () => {
	const dirs = promptSearchDirs("/tmp/project", "/work/extensions/self-compact");
	assert.deepEqual(dirs, ["/tmp/project/.pi/self-compact", "/work/extensions/self-compact/prompts"]);
});

test("loadPromptFile: cwd file wins, then vendored defaults, then builtin", () => {
	const root = mkdtempSync(join(tmpdir(), "sc-prompts-"));
	const cwdDir = join(root, "cwd", ".pi", "self-compact");
	const extDir = join(root, "ext", "prompts");
...
test("the vendored prompt files exist and carry live-value placeholders", () => {
...
test("the real extension layout resolves its vendored defaults without a project .pi directory", () => {
	const extensionDir = resolve(HERE);
	const dirs = promptSearchDirs("/tmp/does-not-exist", extensionDir);
	const loaded = loadPromptFile("compaction", dirs);
	assert.equal(loaded.source, join(extensionDir, "prompts", PROMPT_FILES.compaction));
	assert.match(loaded.text, /## Next Steps/);
});
