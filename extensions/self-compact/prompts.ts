 ...
 *   1. <cwd>/.pi/self-compact/<name>          (project override, user-editable)
 *   2. <extension dir>/prompts/<name>          (defaults vendored with the extension)
 *   3. built-in default
 *
 * LOCAL CHANGE vs upstream: upstream's second directory was `<extension dir>/../../.pi/self-compact`
 * (the working directory that shipped the extension in the upstream sample app). This package
 * vendors the default prompt files under `extensions/self-compact/prompts/` instead, so there is
 * no project-root `.pi/` directory to create here and the defaults travel with the extension.
 */
...
export const BUILTIN_PROMPTS: Record<PromptKind, string> = {
	...
	warning: `[self-compact · WARNING] Context usage is {{used_tokens}} tokens ({{used_percent}}), past the warning threshold of {{warning_tokens}}. If work remains, compact at the next safe boundary: hard cutoff at {{forced_tokens}} ({{forced_percent}}), {{remaining_to_forced}} tokens left. Write your \`note_to_self\` (max {{note_max_chars}} chars: goal, DONE with exact paths, IN PROGRESS, decisions, verified test results, NEXT ACTION last) and call \`self_compact\` alone. If the task is complete, report completion and stop. Diagnostics and cancellation remain available at the hard cutoff.`,
...
export const FORCED_PROMPT = `[self-compact · FORCED] Context usage is {{used_tokens}} tokens ({{used_percent}}), at the hard cutoff of {{forced_tokens}} ({{forced_percent}}). Ordinary work tools are blocked until context is reduced. \`view_context\`, status and cancellation tools, /self-compact-info and /self-compact-cancel remain available. If work remains, write \`note_to_self\` (max {{note_max_chars}} chars: goal, DONE with exact paths and commands, IN PROGRESS, key decisions, verified test results, exact NEXT ACTION last) and call \`self_compact\` alone, or use {} to retry a saved note after addressing the failure. If the task is complete, report completion and stop.`;

export function promptSearchDirs(cwd: string, extensionDir: string): string[] {
	const dirs = [resolve(cwd, PROMPT_SUBDIR), resolve(extensionDir, "prompts")];
