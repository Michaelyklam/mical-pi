...
- Caps and cleanup: `MAX_RUNNING = 16`, `MAX_TRACKED = 64` with LRU pruning of settled
...
- **Concurrency cap**: `MAX_RUNNING = 16` enforced with a synchronous
...
4. **Concurrency cap scope.** Keep one global `MAX_RUNNING = 16`, or per-backend caps?
   A single global cap is the implemented default.
