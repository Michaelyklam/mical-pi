...
test("the 16-session global concurrency cap includes by-the-way sessions", async () => {
  await withManager(async (manager, runtime) => {
    const tasks: SpawnTask[] = [
      { ...task("side question"), origin: "btw" },
      ...Array.from({ length: 15 }, (_, index) => task(`Task ${index + 2}`)),
    ];
...
test("the concurrency cap rejects a 17th running subagent", async () => {
  await withManager(async (manager, runtime) => {
    const spawns = await runTool(
      runtime,
      Effect.forEach(
    ...
    assert.equal(spawns.length, 16);
    await assert.rejects(
      runTool(runtime, manager.spawn("codex", task("Task 17"))),
      /Max 16 subagents/,
    );
  });
});
...
test("idle restarts respect the concurrency cap", async () => {
  await withManager(async (manager, runtime) => {
    ...
    // Restarting the settled one would be a 17th concurrent run.
    await assert.rejects(
