...
export async function showUsageDashboard(ctx: ExtensionCommandContext, entries: DashboardEntry[], activeKey?: string): Promise<DashboardAction> {
	if (entries.length === 0) {
	...
	return ctx.ui.custom<DashboardAction>((tui, theme, _keybindings, done) => {
		const model = new DashboardModel(entries, activeKey, done);
		return {
			invalidate() {},
			...
			render(width: number) {
				...
				const left = model.entries.map((entry) => {
					...
					const status = entry.account.archived ? "Archived" : entry.account.active ? entry.usage.status === "local" ? "Local" : entry.usage.status === "stale" ? "Stale" : "Live" : "Inactive";
